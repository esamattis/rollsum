...
rsync.
     ...
     $Id: pysync.py 1.17 Thu, 02 May 2002 17:05:33 +1000 abo $
...
Requires: sys, zlib, types, md4sum
    adler32 (included md4sum-alike adler32 wrapper)
...
are integers. The sigtable is implemented as a adler32 keyed dictionary of
md4sum keyed dictionaries containing offsets.
ie sigtable[adler32][md4sum]=offset
...
class rdeltaobj:
    ...
    def __init__(self,(length,blocksize,sigtable)):
        ...
        self.sig = None             # the adler32 sig of the next data block
    ...
    def calcdelta(self,newdata):
        ...
        while self.pos+self.blocksize<len(self.data):
            try:
                ...
                # clear the adler32 sig
                self.sig = None
            except KeyError:
                # get the first and last bytes, and append the missed byte
                x1 = self.data[self.pos]
                xn = self.data[self.pos+self.blocksize]

                self._appendmiss(1)
                # rotate the adler32 sig
                self.sig.rotate(x1,xn)
    ...
    def flush(self,mode=R_FINISH):
        ...
        if self.pos < len(self.data):
            try:
                ...
                # append the missed fragment to delta
                self._appendmiss(len(self.data)-self.pos)
...
adler32 keys,collisions : %i %i
""" %  (length,blocksize,blks,
        md4sumkeys,blks - md4sumkeys,
        adler32keys,blks - adler32keys)
